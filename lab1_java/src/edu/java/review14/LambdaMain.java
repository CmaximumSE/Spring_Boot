...
public class LambdaMain {
	...
	public static void main(String[] args) {
		...
		System.out.println(adder.calculator(1, 2));
		
		// 2. 지역 클래스 사용 : 
		// 지역 클래스 정의
		class Subtracrter implements Calculator {
			@Override
		...
		Calculator subtracter = new Subtracrter(); // 지역 클래스 타입 객체 생성
		System.out.println(subtracter.calculator(1, 2));
		...
		System.out.println(multiplier.calculator(1, 2));
		
		// 4. 람다 표현식(lambda expression) - 함수 표기법
		Calculator divider = (x, y) -> x / y;
		// (double x, double y) -> { return x / y; }
		System.out.println(divider.calculator(1, 2));
		...
		/* 
		 	...
		 	람다 표현식 body({ ... })에서 문장이 하나 뿐이면, { } 생략할 수 있다.
		 	body에서 하나 뿐인 문장이 return 문인 경우 { } 와 return 키워드까지 생략함. (파라미터, ...) -> 리턴값;
		 	람다 표현식에서 파라미터가 한 개뿐인 경유에는  ( )를 생략 할 수 있다.
		 	x -> 문장;
