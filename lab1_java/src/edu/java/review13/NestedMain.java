...
/*
	...
	자바에서 내부클래스(또는 인터페이스)를 선언(정의)할 수 있는 위치 :
		1. 멤버 내부 클래스 : 필드를 선언하는 위치에서 선언(정의)하는 클래스.
			(1) static 아닌 내부 클래스 - 외부 클래스 타입의 객체가 생성되어야 사용할 수 있는 클래스.
			(2) static 내부 클래스 - 외부 클래스 타입의 객체 생성 여부와 상관 없이 사용 할 수 있는 클래스.
		...
		(주의) 내부 인터페이스는 static으로만 선언 가능. 보통 public static은 생략.
