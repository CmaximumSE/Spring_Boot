...
/* 
 	static : 멤버(필드, 메서드, 내부 클래스) 선언에서 사용하는 수식어. 지역변수에서는 사용할 수 없음.
 		- 클래스 멤버.
 		- "클래스이름.멤버" 형식으로 사용.
 		- 객체가 생성되기 전에 (프로그램 시작 시점에) 메모리에 로딩되는 클래스 멤버.
 		- 자바 실행 환경(Java Runtime : JRE)이 자바 프로그램을 실행할 때 
 		클래스 로더(class loader)에 의해서 매모리에 로딩되는 클래스 멤버. (JRE 과거에는 JVM) (class loader은 JRE 내부에 존재)
 		- 프로그램 시작 시점부터 프로그램 종료될 때까지 사용할 수 있는 멤버.
 		
	non-static 멤버 : static이 아닌 멤버(필드, 메서드, 내부 클래스). 인스턴스 멤버.
		- 객체(인스터스)를 생성한 후에 참조변수를 이용해서 사용하는 멤버.
...
public class StaticMain {
	...
	public static void main(String[] args) {
		// static 필드/메서드의 예 :
		System.out.println(Math.PI); // static 필드(멤버변수)
		System.out.println(Math.abs(-1)); //static 메서드
		
//		Math math = new Math(); // <-생성자가 private 여서 나는 에러
//		Math.abs(-1);
