...
/* 
 	final 수식어 : 클래스, 멤버(필드, 메서드), 지역 변수 수식. 최종(마지막) -> 변경할 수 없는.
 	1) final 지역 변수 : 값을 한 번만 초기화 할 수 있고, 초기화 이후에는 값을 변경할 수 없는 지역 변수.
 	2) final field(멤버 변수) : 반드시 한 번 초기화를 해야하고, 초기화한 이후에는 값을 변경할 수 없는 필드.
 		(1) 멤버 변수를 선언하는 위치에서 초기화
 		(2) argument를 갖는 생성자에서 초기화
	3) final 메서드 : 변경할 수 없는 메서드. 재정의(override)할 수 없는 메서드.
		상위 클래스에서 정의한 메서드를 하위 클래스에서 재정의 할 수 없다.
...
public class InheritanceMain {
	...
	final String s2; // (2) final 멤버 변수를 생성자에서 초기
