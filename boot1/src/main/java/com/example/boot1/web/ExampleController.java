...
public class ExampleController {
	...
	public String ex1() {
		...
		// -> 템플릿 엔진(thymleaf, mustache, ...)을 사용하는 경우에는,
	...
	// 컨트롤러 메서드에 파라미터를 선언할 때 요청 파라미터(request parameter) 이름과 동일하게 선언하면,
	// DispatcherServle()이 컨트롤러 메서드를 호출할 때 요청 파라미터의 값을 argument 값으로 전달해줌.
	public String ex2(String keyword, Model model) {
		log.info("ex2(keyword={}) 호출", keyword);
		
		// Model :  컨트롤러에서 뷰(view - HTML)에 데이터를 전달할 때 사용하는 객체.
	...
	@GetMapping("/ex3")
	public String ex3(@RequestParam(name = "q", defaultValue = "기본값") String keyword, Model model) {
		// @RequestParam 애너테이션 :
		// (1) 요청 파라미터 이름과 메서드 파라미터 이름이 다를 때 사용.
		// (2) 요청 파라미터 값이 없을 때 사용할 기본값(defaultValue)를 설정할 때 사용.
		log.info("ex3(keyword={}) 호출", keyword);
	...
	//POST 요청 : 요청 파라미터(Request parameter)가 요청 주소(URL)에 보이지 않고, 요청 패킷(HTTP packet)의 body에 포함되어 전달되는 방식.
	
	@PostMapping("/ex4") // POST 방식의 "ex4" 요청 주소를 처리하는 컨트롤러 메서드.
	public String ex4(String userId, String pwd, Model model) {
		log.info("ex4(userId={}, pwd={}) 호출", userId, pwd);
