...
public class PostService {
	...
	public Post update(Long id, PostUpdateDto dto) {
		...
		// 메서드 이름 앞에 @Transactional 애너이션을 사용한 경우,
		// postRepository.save() 메서드를 호출하지 않아도,
		// 트랜잭션이 끝나는 시점(메서드가 종료되는 시점)에 DB 저장이 자동으로 이루어짐.
		// postRepository.save(entity) 메서드 기능 :
		// (1) DB에 저장되지 않은 앤터티인 경우, insert SQL
		// (2) DB에 저장된 엔터티인 경우, update SQL
		
		return entity; // 업데이트가 완료된 엔터티 객체를 리턴.
	}
