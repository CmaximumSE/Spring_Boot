...
// 데이터 베이스 테이블들이 공통으로 사용하는 시간 정보 컬럼(들)을 사용
// 다른 클래스에서 상속하도록 함.

// JPA(Java Persistance API, 자바 영속성 API)의 auditing 기능을 사용하기 위한 설정 : 
...
@MappedSuperclass // BaseTimeEntity를 상속하는 모든 하위 클래스들은 생성시간, 수정시간 컬럼을 갖도록 하기 위해서.
@EntityListeners(AuditingEntityListener.class) // 이벤트(insert update)가 발생할 때 시간을 자동으로 기록.
